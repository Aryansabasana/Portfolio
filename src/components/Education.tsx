...
export default function Education() {
  return (
    <section id="education" className="py-24 px-6 md:px-12 bg-black text-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
         <div className="flex items-end justify-between mb-20 border-b-2 border-white/20 pb-6">
          <h2 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black uppercase tracking-tighter">
            Education<br/>Journey
          </h2>
          <GraduationCap size={48} className="text-white hidden md:block" strokeWidth={1.5} />
        </div>

        <div className="relative">
          {/* Vertical Line */}
          <div className="absolute left-[19px] md:left-1/2 top-0 bottom-0 w-0.5 bg-white/20 md:-translate-x-1/2"></div>
          ...
          <div className="space-y-12 md:space-y-24">
            {educationData.map((item, index) => (
              ...
              >
                {/* Timeline Dot */}
                <div className="absolute left-0 md:left-1/2 top-0 w-10 h-10 bg-black border-2 border-white flex items-center justify-center z-10 md:-translate-x-1/2 shadow-sm">
                    <div className="w-3 h-3 bg-indigo-500"></div>
                </div>

                {/* Content Half */}
                <div className="pl-16 md:pl-0 md:w-1/2 md:px-12">
                   <div className={`bg-[#1a1a1a] p-6 md:p-8 border-2 border-white/20 shadow-hard-sm hover:shadow-hard hover:border-white transition-all duration-300 group relative`}>
                      {/* Connector Line for Desktop */}
                      <div className={`hidden md:block absolute top-5 w-12 h-0.5 bg-white/20 ${index % 2 === 0 ? '-left-12' : '-right-12'}`}></div>

                      <span className="inline-block px-3 py-1 bg-white text-black text-xs font-mono mb-4 uppercase tracking-widest">
                        {item.year}
                      </span>
